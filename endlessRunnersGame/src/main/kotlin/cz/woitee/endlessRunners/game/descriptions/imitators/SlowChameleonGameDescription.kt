...
/**
 * Imitation of Chameleon Run with slower speed, such that it can be played with smaller block size.
 */
class SlowChameleonGameDescription : GameDescription() {
    ...
    override val allActions = arrayListOf(
        ...
        ChangeColorHoldAction(GameObjectColor.GREEN)
    )
    ...
    override val collisionEffects = hashMapOf<BaseCollisionHandler.CollisionHandlerEntry, ICollisionEffect>(
        ...
        Pair(
            ...
            ConditionalCollisionEffect(PlayerHasColor(GameObjectColor.GREEN), ApplyGameEffect(GameOver()), MoveToContact())
        ),
